...
print_r($guarantorsId);
foreach ($guarantersId as $gid) {
	array_push($willsToApprove, json_decode(select_query($con, "gm.*, wm.title, wm.description, um.first_name, um.last_name", "will_guarantor_master gm LEFT JOIN will_master wm ON gm.willId=wm.id LEFT JOIN user_master um on wm.createdBy=um.id", "gm.enabled='1' AND gm.guarantorId=" . $gid->id, "hasApproved='0'", "", ""))[0]);
}

foreach ($beneficiariesId as $bid) {
	$willsAsBeneficiary = json_decode(select_query($con, "wm.title, wm.description, um.first_name, um.last_name", "`will_master` `wm` 
	LEFT JOIN `will_property_beneficiary_master` `pbm` ON `pbm`.`enabled`='1' AND `pbm`.`beneficiaryId` = " . $bid->id . " AND `pbm`.`willId`= `wm`.`id` 
	LEFT JOIN `will_share_beneficiary_master` `sbm` ON `sbm`.`enabled`='1' AND `sbm`.`beneficiaryId` = " . $bid->id . " AND `sbm`.`willId`= `wm`.`id`
	LEFT JOIN `will_bank_beneficiary_master` `bbm` ON `bbm`.`enabled`='1' AND `bbm`.`beneficiaryId` = " . $bid->id . " AND `bbm`.`willId`= `wm`.`id`
	LEFT JOIN `will_fd_beneficiary_master` `fbm` ON `fbm`.`enabled`='1' AND `fbm`.`beneficiaryId` = " . $bid->id . " AND `fbm`.`willId`= `wm`.`id`
	LEFT JOIN `will_insurance_beneficiary_master` `ibm` ON `ibm`.`enabled`='1' AND `ibm`.`beneficiaryId` = " . $bid->id . " AND `ibm`.`willId`= `wm`.`id`
	LEFT JOIN `will_mc_beneficiary_master` `mbm` ON `mbm`.`enabled`='1' AND `mbm`.`beneficiaryId` = " . $bid->id . " AND `mbm`.`willId`= `wm`.`id`
	LEFT JOIN `will_mf_beneficiary_master` `mfbm` ON `mfbm`.`enabled`='1' AND `mfbm`.`beneficiaryId` = " . $bid->id . " AND `mfbm`.`willId`= `wm`.`id`
	LEFT JOIN `user_master` `um` ON `um`.`enabled`='1' AND `um`.`id` = `wm`.`createdBy`", "wm.approved='1'", "", "", ""));
}
